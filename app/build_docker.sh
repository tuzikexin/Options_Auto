...
if [ $? -eq 0 ]; then
    ...
    if [ $(ls -A $credentials_folder | wc -l) -eq 0 ]; then
        ...
        docker run --rm -v $credentials_folder:/app/credentials/ --name temp-vix-container tuzikexin/${IMAGE_NAME}:${IMAGE_TAG} --ticker VIX SPX --start_time_h 8 --start_time_m 55 --end_time_h 16 --end_time_m 5 --test_mode yes
